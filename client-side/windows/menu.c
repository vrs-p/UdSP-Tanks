...
void menu_create(MENU* menu) {
    ...
    vec = {xPosition, SCREEN_HEIGHT / 2 - sfText_getLocalBounds(menu->name).height * 9};
    ...
    vec = {xPosition, SCREEN_HEIGHT / 2 - sfText_getLocalBounds(menu->ipAddressText).height * 7};
    ...
    vec = {xPosition, SCREEN_HEIGHT / 2 - sfText_getLocalBounds(menu->portText).height * 4};
    ...
    sfVector2f vec = {sfText_getLocalBounds(menu->name).width + xSpace, SCREEN_HEIGHT / 2 - sfText_getLocalBounds(menu->name).height * 9};
    txtbox_set_position(text, vec);
    txtbox_set_limit(text, 15);
    txtbox_set_selected(text, true);
    text = ls_iterator_move_next(iterator);
    vec = {sfText_getLocalBounds(menu->ipAddressText).width + xSpace, SCREEN_HEIGHT / 2 - sfText_getLocalBounds(menu->ipAddressText).height * 7};
    txtbox_set_position(text, vec);
    txtbox_set_limit(text, 15);
    txtbox_set_initial_text(text, "158.193.128.160");
    text = ls_iterator_move_next(iterator);
    vec = {sfText_getLocalBounds(menu->portText).width + xSpace, SCREEN_HEIGHT / 2 - sfText_getLocalBounds(menu->portText).height * 4};
    txtbox_set_position(text, vec);
    txtbox_set_limit(text, 5);
...
void menu_destroy(MENU* menu) {
    ...
    ls_run_function(menu->textboxes, txtBox_destroy);
...
void menu_render(MENU* menu) {
    ...
    while (!menu->gameStarted && !menu->appClosed) {
        sfRenderWindow_clear(menu->window);
        ...
        while (ls_iterator_has_next(iterator)) {
            sfText* text = ls_iterator_move_next(iterator);
                ...
                switch (event.key.code) {
                    case sfKeyUp:
                        for (int i = 0; i < ls_get_size(menu->textboxes); ++i) {
                            ...
                            ls_access_at(menu->textboxes, *textbox, i);
                            if (txtbox_is_selected(textbox) && i > 0) {
                                txtbox_set_selected(textbox, false);
                                ls_access_at(menu->textboxes, *textbox, i - 1);
...
bool menu_validate(MENU* menu) {
    TEXTBOX* textbox0 = malloc(sizeof(TEXTBOX*));
    ls_access_at(menu->textboxes, textbox0, 0);
    TEXTBOX* textbox1 = malloc(sizeof(TEXTBOX*));
    ls_access_at(menu->textboxes, textbox1, 1);
    TEXTBOX* textbox2 = malloc(sizeof(TEXTBOX*));
    ls_access_at(menu->textboxes, textbox2, 2);
